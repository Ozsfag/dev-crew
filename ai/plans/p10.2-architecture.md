...
## 2.5 BillingPostAgentHook: N+1 запросы ⚠️ ЧАСТИЧНО
...
**Что сделано**: сигнатура `PostAgentHook` изменена — добавлен `projectId`. Количество запросов
сокращено с 4 до 1 (`getProjectById`). НО `orgId` всё ещё получается через DB lookup.

**Остаток** перенесён в П11.5. Для полного решения нужно добавить `orgId` в сигнатуру
и убрать `OrganizationQueryService` из `BillingPostAgentHook`.

**Acceptance Criteria**:
- [x] `PostAgentHook` получает `projectId` как параметр
- [ ] `PostAgentHook` получает `orgId` как параметр → **П11.5**
- [ ] `BillingPostAgentHook` не делает доп. запросов для org/project lookup → **П11.5**
