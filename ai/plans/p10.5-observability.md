...
**Acceptance Criteria**:
- [x] `MeteredCommandRunner` оборачивает все `@Tool`-операции Timer (`devcrew.tool.execution`)
- [x] Метрики видны в `/actuator/prometheus`
- [x] Тест с `SimpleMeterRegistry` в `MeteredCommandRunnerTest`
...
**Acceptance Criteria**:
- [x] `AgentExecutionService` устанавливает MDC для `taskId`, `agentRole`
- [x] После завершения MDC очищается (`MDC.clear()` в `finally`-блоке)
