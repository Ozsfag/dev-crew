> **Когда читать**: пишешь Java-код — ConfigurationProperties, MapStruct,
> транзакции, Lombok, логирование, стиль, KISS/DRY/YAGNI.
...
#### PII и секреты в логах

В логи **никогда** не попадают в открытом виде: JWT и refresh-токены, токен
сессии Claude, bot-token Telegram, ключи Stripe, пароли.

Специфика dev-crew: **prompt и результат агента** могут содержать код, секреты из
чужого репозитория, ключи. Поэтому:

```java
log.debug("Промпт агента: {}", prompt);        // ❌ полный prompt/результат — только debug, не info/prod
log.info("Агент завершил: taskId={}, len={}", taskId, result.length());  // ✅ метаданные, не содержимое
```

- Полный текст prompt/result — максимум `log.debug`, не `info`/`warn`/`error` и не на проде.
- Идентификаторы (`taskId`, `orgId`) логировать можно; содержимое задачи/ответа — нет.
