...
/**
 * Инструменты для работы с Git-репозиторием. Все пути проверяются через SandboxPolicy. Передаются
 * агенту через LangChain4j AiServices.
 */
@Component
...
public class GitTools {
  ...
  public String getCurrentBranch(String projectPath) {
    try {
      sandboxPolicy.validatePath(projectPath);
      var result =
          commandRunner.run(new File(projectPath), "git", "rev-parse", "--abbrev-ref", "HEAD");
      return result.output();
    } catch (DomainException e) {
      return "ERROR: " + e.getMessage();
    }
