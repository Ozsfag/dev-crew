...
class TelegramNotificationAdapterTest {
    ...
    void send_calls_api_client_when_enabled() {
        ...
        verify(telegramApiClient).sendMessage(org.mockito.ArgumentMatchers.eq(chatId), captor.capture());
    ...
    void onAgentCompleted_sends_formatted_message() {
        ...
        verify(telegramApiClient).sendMessage(
            org.mockito.ArgumentMatchers.eq("999"), captor.capture()
        );
    ...
    void onAgentCompleted_skips_when_disabled() {
        ...
        adapter.onAgentCompleted(UUID.randomUUID(), AgentRole.QA, "result");
