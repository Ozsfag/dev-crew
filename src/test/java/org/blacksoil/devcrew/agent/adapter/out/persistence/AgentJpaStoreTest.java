...
@ActiveProfiles("tc")
@Transactional
class AgentJpaStoreTest {

    @Autowired
    private AgentJpaStore agentJpaStore;
    ...
    void save_updates_existing_agent() {
        ...
        var updated = new AgentModel(
            saved.id(), saved.role(), AgentStatus.RUNNING,
            saved.systemPrompt(), saved.createdAt(), Instant.now()
        );
    ...
    private AgentModel agentModel(AgentRole role) {
        return new AgentModel(
            UUID.randomUUID(), role, AgentStatus.IDLE,
