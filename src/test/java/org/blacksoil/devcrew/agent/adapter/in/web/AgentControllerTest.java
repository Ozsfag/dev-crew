...
class AgentControllerTest {
    ...
    @BeforeEach
    void setUp() {
        var controller = new AgentController(agentQueryService, Mappers.getMapper(AgentWebMapper.class));
    ...
    void GET_agents_returns_200_with_list() throws Exception {
        when(agentQueryService.getAll()).thenReturn(List.of(
            agentModel(AgentRole.BACKEND_DEV),
            agentModel(AgentRole.QA)
        ));
    ...
    void GET_agents_id_returns_200_when_found() throws Exception {
        ...
        mockMvc.perform(get("/api/agents/{id}", id).accept(MediaType.APPLICATION_JSON))
            .andExpect(status().isOk())
